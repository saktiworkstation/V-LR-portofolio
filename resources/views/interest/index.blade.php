    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <x-link-primary-button :url="route('interest.create')" class="bg-green-600 font-thin">{{ __('Add New Interest') }}</x-link-primary-button>
                </div>
                ...
                <div class="max-w-4xl">
                    ...
                    <table class="border-collapse border-slate-500">
                        <thead class="bg-gray-900 text-white">
                            <tr>
                                <th class="px-6 py-2 font-light border border-slate-600">Interest</th>
                                ...
                                @foreach ($datas as $data)
                                    <tr class="text-gray-900">
                                        ...
                                        <td class="px-6 py-2 border border-slate-700">
                                            <x-link-small-button :url="url('/interest/' . $data->id . '/edit')"  class=" w-[75px] h-[30px] mb-2">
                                                {{ __('Edit') }}
                                            </x-link-small-button>
                                            <form action="/interest/{{ $data->id }}/delete" method="post"
                                                ...
                                                @method('delete')
                                                @csrf
                                                <x-delete-button :message="$data->interest" class=" w-[75px] h-[30px] bg-red-600 ">
                                                    Delete
                                                </x-delete-button>
                                            </form>
                                        </td>
