    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <x-link-primary-button :url="route('skill.create')" class="bg-green-600 font-thin">{{ __('Add New Skill') }}</x-link-primary-button>
                </div>
                ...
                <div class="w-full h-[500px]">
                    ...
                    <table class="border-collapse border-slate-500 w-full">
                        <thead class="bg-gray-900 text-white">
                            <tr>
                                <th class="px-6 py-2 font-light border border-slate-600">skill Name</th>
                                <th class="px-6 py-2 font-light border border-slate-600">Desciption</th>
                                <th class="px-6 py-2 font-light border border-slate-600">Skill Level</th>
                        ...
                        <tbody class="bg-slate-200">
                            @if ($datas->count() > 0)
                                @foreach ($datas as $data)
                                    <tr class="text-gray-900">
                                        ...
                                        <td class="px-6 py-2 border border-slate-700">
                                            <x-link-small-button :url="url('/skill/' . $data->id . '/edit')" class=" w-[75px] h-[30px] mb-2">
                                                {{ __('Edit') }}
                                            </x-link-small-button>
                                ...
                                @endforeach
                            @else
                                <h4 class="pb-3 font-light text-base text-red-500 leading-tight">
